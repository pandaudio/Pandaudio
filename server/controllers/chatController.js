...
chatController.getAll = async (req, res, next) => {
  try {
    const { roomId } = req.params;
    query = `SELECT users.username, users.thumbnail, chat${roomId}.content, chat${roomId}.created_at FROM chat${roomId} INNER JOIN users ON CAST(chat${roomId}.owner as uuid)=users.id ORDER BY created_at DESC LIMIT 20`;
